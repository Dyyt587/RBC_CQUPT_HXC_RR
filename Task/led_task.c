...
// 操作函数1
int auto_action01_pushring(int condition) 
{ 	
	//int satisfied;
    // 模拟条件判断，返回值表示是否满足条件
		if(condition == 1)
...
// 操作函数2
int auto_action02_risering(int condition) {
...
// 操作函数3
int auto_action03_hitring(int condition) {
    if (condition == 3) {
...
int auto_action04_allreset(int condition) {
    if (condition == 4) {
...
void led_task(void const * argument)
{
 ...
 // 初始化条件
		int condition=0;
		condition=set_condition_start(condition);
    // 执行操作1
    auto_action01_pushring(condition);
		condition=set_condition_judge(condition);
    // 在操作1之后执行操作2
